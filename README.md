...
- First Load JS 152 kB (`/` 64.1 kB). Below-fold workflow views (analysis, compare, Q&A, export memo) load via `next/dynamic` with `ssr: false` — never in the initial chunk.
...
- Evergreen-only browserslist (Chrome ≥ 94, Firefox ≥ 93, Safari ≥ 15.4) + `es2022` TS target — no legacy transforms for app code.
