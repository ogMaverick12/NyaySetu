...
- First Load JS 122 kB (`/` 33.7 kB). Below-fold workflow views (analysis, compare, Q&A, export memo) load via `next/dynamic` with `ssr: false` — never in the initial chunk. Post-extraction UI (clause panel, fairness card) streams on demand too, and page-level imports bypass feature barrels so server-only `zod` schemas (149 KB) stay out of the client entirely.
...
- Evergreen-only browserslist (Chrome ≥ 94, Firefox ≥ 93, Safari ≥ 15.4) + `es2022` TS target — no legacy transforms for app code. The only legacy-JS flag left is Next.js's fixed runtime polyfill chunk (platform floor, not app code). IBM Plex Sans ships without the unused Light weight, and IBM Plex Mono (labels only, never hero text) loads with `preload: false` so it stays out of the critical request chain.
